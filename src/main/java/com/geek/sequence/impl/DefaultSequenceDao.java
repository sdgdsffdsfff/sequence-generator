package com.geek.sequence.impl;
...
import com.geek.sequence.SequenceDao;
import com.geek.sequence.SequenceRange;
import com.geek.sequence.exceptions.SequenceException;

/**
 * 序列DAO默认实现，JDBC方式
 * 
 * @author 机冷
 * @version $Id: DefaultSequenceDao.java, v 0.1 2015年5月11日 上午12:17:03 机冷 Exp $
 */
public class DefaultSequenceDao implements SequenceDao{
    ...
    /**
     * get out of spring container
     * @author 机冷
     */
    public DefaultSequenceDao(){
        Map<?, ?> config;
        try {
            config = Yaml.loadType(ClassLoader.getSystemResourceAsStream("sequence-jdbc.yml"), HashMap.class);
        } catch (FileNotFoundException e) {
            throw new RuntimeException("can't get sequence-jdbc.yml......");
        }
        config = (Map<?,?>) ((Map<?,?>) config.get("sequence")).get("jdbc");
        //初始化数据源
        dataSource = new DriverManagerDataSource((String) config.get("url"), (String) config.get("username"),
